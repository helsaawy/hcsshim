...
// G returns a `logrus.Entry` stored in context (if any), appended with the
// `TraceID, SpanID` from `ctx` if `ctx` contains an OpenCensus `trace.Span`.
//
...
func G(ctx context.Context) *logrus.Entry {
	...
	//TODO: remove this and modify the etw hooks to extract span info:
	//  `trace.FromContext(entry.Context)`
...
// S updates the log entry in the context with the provided fields, and returns
// both. It is equivlent to:
//   entry := G(ctx).WithFields(fields)
//   ctx = WithContext(ctx, entry)
func S(ctx context.Context, fields logrus.Fields) (context.Context, *logrus.Entry) {
	e := G(ctx)
	if len(fields) > 0 {
