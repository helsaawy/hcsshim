...
func newProcess(c *Container, spec *oci.Process, process runtime.Process, pid uint32, init bool) *containerProcess {
	...
	p.writersWg.Add(1)
	go func() {
		ctx, span := oc.StartTraceSpan(context.Background(), "newProcess::waitBackground")
		defer span.End()
		span.AddAttributes(
		...
		go func() {
			...
			_, span := oc.StartTraceSpan(context.Background(), "newProcess::waitBackground::waitAllWaiters")
			defer span.End()
			span.AddAttributes(
				trace.StringAttribute("cid", p.cid),
				trace.Int64Attribute("pid", int64(p.pid)))
...
func (p *containerProcess) Wait() (<-chan int, chan<- bool) {
	ctx, span := oc.StartTraceSpan(context.Background(), "opengcs::containerProcess::Wait")
	span.AddAttributes(
		trace.StringAttribute("cid", p.cid),
		trace.Int64Attribute("pid", int64(p.pid)))
...
func (ep *externalProcess) Wait() (<-chan int, chan<- bool) {
	_, span := oc.StartTraceSpan(context.Background(), "opengcs::externalProcess::Wait")
