...
func (b *Bridge) ListenAndServe(bridgeIn io.ReadCloser, bridgeOut io.WriteCloser) error {
	...
	go func() {
		...
		for {
			if atomic.LoadUint32(&b.hasQuitPending) == 0 {
				...
				ctx := base.ExtractContext(context.Background())
				ctx, _ = otel.StartSpan(ctx,
					otel.Name("opengcs", "bridge", header.Type.String()),
					otel.WithServerSpanKind,
					trace.WithAttributes(
						semconv.RPCSystemKey.String("gcs"),
						attribute.Int64("message-id", int64(header.ID)),
						attribute.String("activityID", base.ActivityID.String()),
						attribute.String(logfields.ContainerID, base.ContainerID),
					))
				...
				requestChan <- &Request{
					...
					ActivityID:  base.ActivityID.String(),
					Message:     message,
					Version:     b.protVer,
				}
			}
	...
	go func() {
		...
		for resp := range b.responseChan {
			...
			if span := trace.SpanFromContext(resp.ctx); span != nil {
				span.End()
			}
		}
...
func (b *Bridge) PublishNotification(n *prot.ContainerNotification) {
	ctx, span := otel.StartSpan(context.Background(),
		otel.Name("opengcs", "bridge", "PublishNotification"),
		otel.WithClientSpanKind)
	span.SetAttributes(attribute.String("notification", log.Format(ctx, n)))
