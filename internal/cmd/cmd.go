...
func (c *Cmd) Start() (err error) {
	...
	ctx, span := oc.StartTraceSpan(pctx, "cmd::Start")
	defer span.End()
	defer func() { oc.SetSpanStatus(span, err) }()
	span.AddAttributes(trace.StringAttribute("cmd", fmt.Sprintf("%q", c.Spec.Args)),
		trace.BoolAttribute("isOCI", c.Host.IsOCI()),
		trace.StringAttribute("os", c.Host.OS()),
		trace.StringAttribute("cwd", c.Spec.Cwd),
		trace.StringAttribute("env", fmt.Sprintf("%q", c.Spec.Env)))
...
func (c *Cmd) Wait() (err error) {
	...
	_, span := oc.StartTraceSpan(pctx, "cmd::Wait")
	defer span.End()
	defer func() { oc.SetSpanStatus(span, err) }()
	span.AddAttributes(trace.StringAttribute("cmd", fmt.Sprintf("%q", c.Spec.Args)))
